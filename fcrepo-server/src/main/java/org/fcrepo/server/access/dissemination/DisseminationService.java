...
public class DisseminationService {
	...
	static {
		try {
			...
			if (fedoraHome == null) {
				...
				String dsMediation = s_server.getModule("org.fcrepo.server.access.Access").getParameter("doMediateDatastreams");
	 ...
	 */
	public MIMETypedStream assembleDissemination(Context context, String PID, Hashtable<String, String> h_userParms,
			DisseminationBindingInfo[] dissBindInfoArray, String deploymentPID, ServiceDeploymentReader bmReader, String methodName) throws ServerException {
		...
		if (dissBindInfoArray != null && dissBindInfoArray.length > 0) {
			...
			for (int i = 0; i < dissBindInfoArray.length; i++) {
				((Authorization) s_server.getModule("org.fcrepo.server.security.Authorization")).enforce_Internal_DSState(context, dissBindInfoArray[i].dsID,
				...
				// takes parameters.
				if (dissBindInfo.dsLocation != null && (dissBindInfo.dsLocation.startsWith("http://") || dissBindInfo.dsLocation.startsWith("https://"))) {
				...
				if (i == 0) {
					...
					if (dissBindInfo.AddressLocation.equalsIgnoreCase(LOCAL_ADDRESS_LOCATION)) {
						 ...
						 * the file
						 */
						dissURL = dissURL.replaceAll(fedoraServerHost + ":" + fedoraServerPort + "/fedora/", fedoraServerHost + ":" + fedoraServerPort + "/"
				...
				Hashtable<String, String> beHash = m_beSS.getSecuritySpec(callbackRole, methodName);
				boolean callbackBasicAuth = new Boolean(beHash.get("callbackBasicAuth")).booleanValue();
				boolean callbackSSL = new Boolean(beHash.get("callbackSSL")).booleanValue();
				...
				if (nextKey.equalsIgnoreCase(currentKey) & i != numElements) {
					...
					if (doDatastreamMediation && !dissBindInfo.dsControlGroupType.equalsIgnoreCase("R")) {
						...
						replaceString = datastreamResolverServletURL
								+ registerDatastreamLocation(dissBindInfo.dsLocation, dissBindInfo.dsControlGroupType, callbackRole, methodName) + "+("
								+ dissBindInfo.DSBindKey + ")";
					} else {
						// Bypass Datastream Mediation.
						if (dissBindInfo.dsControlGroupType.equalsIgnoreCase("M") || dissBindInfo.dsControlGroupType.equalsIgnoreCase("X")) {
							// Use the Default Disseminator syntax to resolve
							// the internal
							// datastream location for Managed and XML
							// datastreams.
							replaceString = resolveInternalDSLocation(context, dissBindInfo.dsLocation, dissBindInfo.dsCreateDT, dsMediatedCallbackHost) + "+("
									+ dissBindInfo.DSBindKey + ")";
							;
						} else {
					...
					if (doDatastreamMediation && !dissBindInfo.dsControlGroupType.equalsIgnoreCase("R")) {
						...
						// datastreams)
						replaceString = datastreamResolverServletURL
								+ registerDatastreamLocation(dissBindInfo.dsLocation, dissBindInfo.dsControlGroupType, callbackRole, methodName); // this
																																					// is
																																					// generic,
																																					// should
																																					// be
				...
				try {
					...
					// URLEncode each parameter before substitution. Otherwise,
					// the
					// operationLocation has no parameters (i.e., it is a simple
					// URL )
					...
					String message = "[DisseminationService] An error occured. The error " + "was \"" + uee.getClass().getName() + "\"  . The Reason was \""
			...
			for (DeploymentDSBindRule element : rules) {
				...
				if (dissURL.indexOf(rulePattern) != -1) {
					throw new DisseminationException(null, "Data Object " + PID + " missing required datastream: " + element.bindingKeyName, null, null, null);
				}
			}
			...
			while (e.hasMoreElements()) {
				...
				try {
					...
					String message = "[DisseminationService] An error occured. The error " + "was \"" + uee.getClass().getName() + "\"  . The Reason was \""
							+ uee.getMessage() + "\"  . Parameter name: " + name + "  . " + "Parameter value: " + value + "  .";
					logger.error(message);
					throw new GeneralException(message);
				}
				String pattern = "\\(" + name + "\\)";
			...
			if (protocolType.equalsIgnoreCase("http")) {
				...
				if (isRedirect) {
					...
					try {
						...
						String message = "[DisseminationService] An error has occurred. " + "The error was a \"" + uee.getClass().getName() + "\"  . The "
								+ "Reason was \"" + uee.getMessage() + "\"  . String value: " + dissURL + "  . ";
						logger.error(message);
						throw new GeneralException(message);
					}
					...
					Hashtable<String, String> beHash = m_beSS.getSecuritySpec(beServiceRole, methodName);
					boolean beServiceCallSSL = new Boolean(beHash.get("callSSL")).booleanValue();
					String beServiceCallUsername = "";
					String beServiceCallPassword = "";
					boolean beServiceCallBasicAuth = new Boolean(beHash.get("callBasicAuth")).booleanValue();
	 ...
	 */
	public String registerDatastreamLocation(String dsLocation, String dsControlGroupType, String beServiceCallbackRole, String methodName)
			throws ServerException {
		...
		try {
			...
			if (tempID == null) {
				...
				// itself.
				String beServiceRole = null;
				if (ServerUtility.isURLFedoraServer(dsLocation) || dsControlGroupType.equals("M") || dsControlGroupType.equals("X")) {
					beServiceRole = BackendPolicies.FEDORA_INTERNAL_CALL;
				} else {
				...
				Hashtable<String, String> beHash = m_beSS.getSecuritySpec(beServiceRole, methodName);
				boolean beServiceCallbackBasicAuth = new Boolean(beHash.get("callbackBasicAuth")).booleanValue();
				boolean beServiceCallBasicAuth = new Boolean(beHash.get("callBasicAuth")).booleanValue();
				boolean beServiceCallbackSSL = new Boolean(beHash.get("callbackSSL")).booleanValue();
				boolean beServiceCallSSL = new Boolean(beHash.get("callSSL")).booleanValue();
			...
			throw new DisseminationException("[DisseminationService] register" + "DatastreamLocation: " + "returned an error. The underlying error was a "
	...
	private String resolveInternalDSLocation(Context context, String internalDSLocation, Date dsCreateDT, String callbackHost) throws ServerException {
		...
		if (callbackHost == null || callbackHost.equals("")) {
			throw new DisseminationException("[DisseminationService] was unable to " + "resolve the base URL of the Fedora Server. The URL specified was: \""
		...
		if (s.length == 3) {
			dsLocation = callbackHost + "/" + fedoraAppServerContext + "/get/" + s[0] + "/" + s[1] + "/" + DateUtility.convertDateToString(dsCreateDT);

		} else {
			String message = "[DisseminationService] An error has occurred. " + "The internal dsLocation: \"" + internalDSLocation + "\" is "
					+ "not in the required format of: " + "\"doPID+DSID+DSVERSIONID\" .";
			logger.error(message);
			throw new GeneralException(message);
		}
